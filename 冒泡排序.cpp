...
void bubble(int arr[],int sz)//本质上为int*arr才对。用的是指针。 
{
	int i = 0;
	for (i = 0; i < sz-1; i++)//sz要减一，是因为10个数字进行冒泡排序时，仅仅只需要比较9次
	{
		   int flag = 1;//flag = 1意味着我们假设某次传递过来的数列已经符合冒泡排序的要求了
		int j = 0;
		for (j = 0; j < sz-1-i; j++)
		//sz-1-i是因为，当有一个数通过冒泡排序来到最右侧时，下一个数进行冒泡排序时可以已知比上一个数字小，不再需要比较，可以节省算力
		{
		   ...
		   if (flag == 1)//说明经过for（j）的循环，没有发生任何的交换，该数据已经符合要求
		  {
			   break;//打破循环，无需下一步.注意：这里的break虽然是在if语句内部，但是if在for（i)的循环内，break的其实是for（i)循环
		  }
	}
}
